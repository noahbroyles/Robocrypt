...
def encrypt(message: bytes, password: bytes, shift: int = None):
    ...
    if shift is not None:
        return cipher_decipher(encrypted_bytes, shift, action='cipher')
...
def decrypt(message: bytes, password: bytes, shift: int = None):
    ...
    if shift is not None:
        message = cipher_decipher(message, shift, action='decipher')
    try:
...
"""
    ...
    if not Path(args.file).exists():
        ...
        sys.exit(1)
    password = getpass.getpass(f"Enter password to {action}crypt: ")
    if action == 'en':
        encrypt_file(args.file, password, shift=args.shift)
    elif action == 'de':
        decrypt_file(args.file, password, shift=args.shift)
